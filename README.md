...
A small but realistic **microservices** Inventory Management System built in Java 17 /
Spring Boot 3. It's sized to be readable end-to-end in an interview setting, while still
demonstrating the core building blocks of scalable, resilient service-to-service
communication: service discovery, an API gateway, synchronous calls guarded by
circuit-breaker/retry/timeout, asynchronous event-driven communication via Kafka,
idempotency, optimistic locking, and a manual saga with compensation.
